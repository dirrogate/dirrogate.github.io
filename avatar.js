...
window.ensureSceneReady = function() {
    if (!xrScene) {
        ...
        window.compassContainer.userData.isEnvironmentDecor = true;
...
function getRoleTint(isAgent, role) {
    if (isAgent) return 0x9b72f0;            
    if (role === "ar_human") return 0x00e5ff; 
    return 0xb24bf3;                          
}
...
window.createShadedEnvironment = function() {
    ...
    floor.userData.isEnvironmentDecor = true;
    xrScene.add(floor);

    const grid = new THREE.GridHelper(8, 16, 0x4fd1c5, 0x4fd1c5);
    grid.position.y = 0.01;
    grid.userData.isEnvironmentDecor = true;
    xrScene.add(grid);
    ...
    walls.userData.isEnvironmentDecor = true;
    xrScene.add(walls);
    ...
    table.userData.isEnvironmentDecor = true;
    xrScene.add(table);
};
...
window.startWebXR = async function() {
    try {
        if (!xrRenderer) {
            xrRenderer = new THREE.WebGLRenderer({ antialias: true, alpha: true, powerPreference: "high-performance", preserveDrawingBuffer: true });
            window.xrRenderer = xrRenderer;
        ...
        if (window.clientRole === "remote_human") {
            ...
            window.xrGLBinding = null;
            window.arCameraAccessSupported = false;
            if (typeof XRWebGLBinding !== 'undefined') {
                try {
                    const gl = xrRenderer.getContext();
                    window.xrGLBinding = new XRWebGLBinding(session, gl);
                    window.arCameraAccessSupported = true;
                    window.updateLog("[Vision] Raw camera access binding created — availability confirmed per-frame.");
                } catch (e) {
                    window.updateLog(`[Vision] Raw camera access unavailable on this device: ${e.message}`);
                }
            } else {
                window.updateLog("[Vision] XRWebGLBinding not supported by this browser — Snap will use the native camera fallback.");
            }
            ...
            xrRenderer.setAnimationLoop((timestamp, frame) => {
                ...
                if (frame && window._pendingARCapture && window.tryCaptureARCameraFrame) {
                    window._pendingARCapture = false;
                    window.tryCaptureARCameraFrame(frame, xrRenderer.xr.getReferenceSpace(), xrRenderer.getContext(), xrRenderer, xrScene, xrCamera);
                }
